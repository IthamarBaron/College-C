...
void array_save_to_file(Array* parray, char* file_name)
{
	...
	/* write the book list to the file */
	for (size_t i = 0; i <parray->count ; i++)
	{
		book_save_to_file(fp, parray->array[i]);
	}
	fclose(fp);
}
...
void array_load_from_file(Array* parray, char* file_name)
{
	...
	if ((fp = fopen(file_name, "rt")) == NULL)
		return;
	/* read the book array from the file */
	fgets(temp, MAX_STR_LEN, fp); /* read first heading line */
	while (!feof(fp))
	{
		BookPtr tempBook = (Book*)malloc(sizeof(Book));
		book_read_from_file(fp, tempBook);
		array_add_book(parray, tempBook);
	}
	fclose(fp);
}
void array_check_for_save(Array* parray, char* file_name)
{
	if (parray->changed_fl)
		array_save_to_file(parray, file_name);
}
